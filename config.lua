...
function TriggerCallback(event, cb, ...)
    ...
    TriggerServerEvent("lb-picchat:cb:" .. event, requestId, ...)
end
...
RegisterNetEvent("lb-picchat:cb:response", function(requestId, ...)
...
function SendReactMessage(event, data)
    exports["lb-phone"]:SendCustomAppMessage("lb-picchat", {
        action = event,
        data = data
    })
end
